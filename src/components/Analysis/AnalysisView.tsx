        ...
        <div className="flex justify-between gap-20 items-start pl-10 pt-20 container mx-auto h-[calc(100vh-78px)] ">
          {isSubmit && (
            <form
              ...
              className=" w-fit container mx-auto flex flex-col"
            >
              <div className="flex flex-col absolute z-10 gap-10 items-center">
                <span className="font-inter text-white text-[28px] font-semibold">
                  Enter Symbol
                </span>
          ...
          <div className="flex flex-col justify-center gap-4 absolute z-10">
            <span className="text-white font-inter font-semibold text-[32px]">
              Analysis View
            </span>
            <div className="flex flex-col gap-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col gap-[2px]">
                  ...
                  <span className="text-[#666668] font-inter font-medium text-[14px]">
                ...
                {AnalysisViewRegion.map((item, index) => {
                  return (
                    <div
                      key={index}
                      className={`${
                        activeIndex === item.id ? "bg-[#3ca1ff]" : ""
                      } cursor-pointer rounded-[32px] px-3 py-2 flex gap-4`}
                      onClick={() => {
                        setActiveIndex(item.id),
                          setIsSubmit(true),
                          setIsMarket(item.port);
                      }}
                    >
                      <div>
                        <Image src={item.image} alt="" width={20} height={20} />
                      </div>
                      <span className="text-white font-inter font-medium text-[16px]">
                        {item.title}
                      </span>
                    </div>
                  );
