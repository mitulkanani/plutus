...
const DerivativeSavedData = ({
  ...
  const [PutOptionsData, setPutOptionsData] = useState(() => {
    if (typeof window !== "undefined") {
      return (
        JSON.parse(localStorage.getItem("derivativePutOptionsData") as any) ||
        []
      );
    } else {
      return []; // Or any other default value you want
    ...
    <>
      {isDerivativeSavedData && (
        <div className="bg-[#1c1c21] relative flex justify-center min-h-[calc(100vh-78px)]">
          <div className="pt-10 container flex justify-center relative z-50 mx-auto">
            <div className="flex gap-16">
              <div className="pt-3">
                <Link
                  className="flex gap-3"
                  href={"/portfolio"}
                  onClick={() => setIsDerivativeSavedData(false)}
                >
                  <Image
