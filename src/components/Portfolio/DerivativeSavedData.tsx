...
const DerivativeSavedData = ({ isDerivativeSavedData, setIsDerivativeSavedData }: any) => {
    const [callOption, setCallOption] = useState(1);
    const [CallOptionsData, setCallOptionsData] = useState(() => {
        if (typeof window !== 'undefined') {
            return JSON.parse(localStorage.getItem("derivativeCallOptionsData") as any) || [];
        } else {
    ...
    const [PutOptionsData, setPutOptionsData] = useState(() => {
        if (typeof window !== 'undefined') {
            return JSON.parse(localStorage.getItem("derivativePutOptionsData") as any) || [];
        } else {
            return []; // Or any other default value you want
        }
    });
                ...
                <div className='bg-[#1c1c21] relative flex justify-center items-center'>
                    <div className='pt-10 container flex justify-center h-[calc(100vh+78px)]  mx-auto'>
                        <div className='absolute z-10 flex gap-16'>
                            <div className='pt-3'>
                                <Link className='flex gap-3' href={"/portfolio"} onClick={() => setIsDerivativeSavedData(false)}>
                                    <Image
...
export default DerivativeSavedData
