...
const PortfolioInputForm = () => {
  ...
  const formik: any = useFormik({
    ...
    onSubmit: (values) => {
      ...
      Derivative.market("/portfolio/analysis")
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
      ...
      fields.push(
        <div key={i} className="flex flex-col gap-x-10 gap-y-5">
          <div className="flex w-[400px] flex-col gap-2">
            <label
              htmlFor={`stock_${i}`}
              className="text-[#9a9a9c] text-[16px] font-inter font-medium"
            >
              Stock Symbol {i + 1}
            </label>
            <input
  ...
  return (
    <div>
      {isSubmit ? (
          ...
          <div className="container relative z-50 flex justify-center pt-10 mx-auto px-[15px] ">
            <div className=" container mx-auto flex flex-col">
              ...
              <form
                ...
                className=" pt-10 flex flex-col items-center gap-12"
              >
                <div className="flex flex-col gap-10 items-center">
                  <span className="font-inter text-white 2xl:text-[38px] text-[32px] font-semibold">
                    Stock Portfolio input form
                  </span>
                  <div className="flex flex-col gap-x-10 gap-y-5">
                    <div className="flex w-[400px] flex-col gap-2">
                      <label
                        ...
                        className="text-[#9a9a9c] text-[16px] font-inter font-medium"
                      >
                        Enter the number of years in consideration
                      </label>
                      <input
                        id="years"
                        name="years"
                    ...
                    <div className="flex w-[400px] flex-col gap-2">
                      <label
                        ...
                        className="text-[#9a9a9c] text-[16px] font-inter font-medium"
                      >
                        Enter how many stocks your portfolio has
                      </label>
                      <input
                        id="num_stocks"
                        name="num_stocks"
                    ...
                    <div className="flex w-[400px] flex-col gap-2">
                      <label
                        ...
                        className="text-[#9a9a9c] text-[16px] font-inter font-medium"
                      >
                        Enter the number of days in consideration
                      </label>
                      <input
                        id="days"
                        name="days"
                  ...
                  <button
                    type="submit"
                    className="text-white w-[100px] font-inter font-medium text-[18px] px-[20px] py-[8px] bg-[#3ca1ff] rounded-[8px]"
                  >
                    {isSpinner ? (
        ...
        <DerivativeSavedData
          isDerivativeSavedData={isDerivativeSavedData}
          setIsDerivativeSavedData={setIsDerivativeSavedData}
        />
      ) : (
          ...
          <div className="h-[calc(100vh-78px)] pt-10 container mx-auto flex flex-col items-center gap-12">
            <div className="flex flex-col absolute z-10 pt-20 w-fit gap-20">
              <div className="flex flex-col gap-4">
                <div
                  className={`bg-[#369eff] cursor-pointer rounded-[32px] px-[20px] py-[8px]`}
                  onClick={() => {
                    setIsLogin(false), setIsSavedData(true);
                  }}
                >
                  <span className=" font-medium flex justify-center text-[16px                                                font-inter">
                    Saved Equity Picks
                  </span>
                </div>
                <div
                  className={`bg-[#369eff] cursor-pointer rounded-[32px] px-[20px] py-[8px]`}
                  onClick={() => {
                    setIsLogin(true), setIsDerivativeSavedData(true);
                  }}
                >
                  <span className=" font-medium text-[16px] font-inter">
                    Saved Derivative Picks
                  </span>
                </div>
              </div>
              <div className="cursor-pointer" onClick={() => setIsForm(true)}>
                <span className="flex justify-center rounded-[32px] font-inter border border-[#369eff] px-[20px] py-[8px]">
                  Risk Analysis
                </span>
              </div>
            </div>
